...
async function runCamelCaseSchema() {
  let connection;
  try {
    // Create connection
    ...
    // Create farmers table with camelCase field names to match backend INSERT
    ...
    await connection.execute(`
      CREATE TABLE farmers (
        ...
        -- Add snake_case aliases for queries
        registration_date DATE GENERATED ALWAYS AS (registrationDate) VIRTUAL,
    ...
    await connection.execute(`
      CREATE TABLE farms (
        ...
        -- Add snake_case aliases for backend compatibility
    ...
    console.log('\n🎉 CamelCase compatible database schema created successfully!');
    console.log('\n📊 Tables created:');
    console.log('  - users');
    console.log('  - farmers (registrationDate for INSERT, registration_date for SELECT)');
    console.log('  - farms (farmerId for INSERT, farmer_id for SELECT)');
